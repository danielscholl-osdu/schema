Feature: To verify functionality of GET schema Service
  ...
  @BugFixRequired #@BUG 477948-[Schema Service] : Wrong error message is shown when invalid authorization value is given
  Scenario Outline: Verify that Schema Service's GET API throws correct exception when auth token is invalid
    ...
    Examples: 
      | SchemaId                                       | ReponseStatusCode | ResponseMessage                                             |
      | "SchemaSanityTest:testsource:testentity.1.1.0" | "401"             | "/output_payloads/SchemaPost_InvalidExpiredTokenError.json" |
