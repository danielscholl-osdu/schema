Feature: To verify functionality of POST schema Service
  ...
  Scenario Outline: Verify whether schema can not be registered with already existing major, but increased minor version
    Given I hit schema service POST API with <InputPayload> and data-partition-id as <tenant> with increased minor version only
    Then service should respond back with error <ReponseStatusCode> and <ResponseMessage>

    Examples: 
      | InputPayload                                           | ReponseStatusCode | ResponseMessage                                        | tenant    |
      | "/input_payloads/inputPayloadWithExistingVersion.json" | "400"             | "/output_payloads/SchemaPost_BreakingChangeError.json" | "opendes" |
