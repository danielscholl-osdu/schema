...
package org.opengroup.osdu.schema.destination.resolver.osm;
...
@Component
@ConditionalOnProperty(value = "osmDriver", havingValue = "datastore")
@Slf4j
@RequiredArgsConstructor
public class DatastoreDestinationResolver implements DsDestinationResolver {
    ...
    public DsDestinationResolution resolve(Destination destination) {
        ...
        Datastore datastore = datastoreCache.computeIfAbsent(partitionId, partitionKey ->
                getDatastoreFor(destination, partitionKey, projectId));
    ...
    private synchronized Datastore getDatastoreFor(Destination destination, String partitionId, String projectId) {
        return datastoreCache.computeIfAbsent(partitionId, partitionKey ->
